...
These tests mirror the ones in ``../tests/test_float_waveform_from_many_files.rs``
"""
...
def test_all_same_file_1():
    batch = FloatWaveform.from_many_files(ALL_SAME_FILENAMES)
...
def test_different_filenames_1():
    batch = FloatWaveform.from_many_files(ALL_FILENAMES)
...
def test_file_not_found_error_1():
    batch = FloatWaveform.from_many_files([COF_FILENAME, "asdfasdf"])
    assert 2 == len(batch)
