...
type Resampler = fn(u32, u32, u32, &[f32]) -> Result<Vec<f32>, babycat_lib_resample::errors::Error>;
...
fn load_wav_file(filename: &str) -> Vec<f32> {
...
fn benchmark_all_funcs(
    ...
    let (lanczos_rms, lanczos_microseconds) = benchmark_func(
        babycat_lib_resample::lanczos::resample,
        input_frame_rate_hz,
        output_frame_rate_hz,
        num_channels,
        &input,
    ...
    let (libsamplerate_rms, libsamplerate_microseconds) = benchmark_func(
        babycat_lib_resample::libsamplerate::resample,
        input_frame_rate_hz,
        output_frame_rate_hz,
        num_channels,
        &input,
...
fn main() {
    ...
    let blippy_trance: Vec<f32> = load_wav_file("audio-for-tests/blippy-trance/track.wav");
