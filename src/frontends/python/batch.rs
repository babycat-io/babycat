...
pub struct BatchProcessor {
    inner: crate::backend::BatchProcessor,
}

impl From<crate::backend::BatchProcessor> for BatchProcessor {
    fn from(inner: crate::backend::BatchProcessor) -> BatchProcessor {
        BatchProcessor { inner }
    }
}

impl std::fmt::Display for BatchProcessor {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        write!(
            f,
            "<babycat.BatchProcessor: {} current workers>",
            self.inner.current_num_workers()
        )
    }
}

#[pyproto]
impl PyObjectProtocol for BatchProcessor {
...
impl BatchProcessor {
    ...
    pub fn new(num_workers: usize) -> Self {
        ...
        let inner = crate::backend::BatchProcessor::new(batch_args);
        Self { inner }
    }

    /// Returns the number of currently active worker threads in the
    /// thread pool.
    ...
    pub fn waveforms_from_files(
        ...
        self.inner
            .from_files(&filenames_ref, waveform_args)
            .into_iter()
    ...
    pub fn waveforms_from_files_to_numpy(
        ...
        let arrays: Vec<ndarray::Array2<f32>> = self.inner.run_in_pool(|| {
            filenames
                .par_iter()
                .map(|filename| {
