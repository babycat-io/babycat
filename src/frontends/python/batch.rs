...
#[allow(clippy::too_many_arguments)]
pub fn waveforms_from_files_with_bytes_filenames_to_numpy_arrays_unwrapped(
    ...
    let waveforms: Vec<Waveform> = thread_pool.install(|| {
        filenames
            ...
            .map(|filename| {
                ...
                Waveform::from_file(decoded_filename, waveform_args).unwrap()
            })
            .collect()
    });
    let waveform_arrays: Vec<Py<PyArray2<f32>>> = waveforms.into_iter().map(|w| w.into()).collect();
...
",
    ...
    batch_submodule.add_function(wrap_pyfunction!(
        waveforms_from_files_with_bytes_filenames_to_numpy_arrays_unwrapped,
        batch_submodule
    )?)?;
