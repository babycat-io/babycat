...
use babycat_lib_resample::resample;
#[macro_use]
pub mod errors;
...
fn milliseconds_to_frames(frame_rate_hz: u32, duration_milliseconds: u64) -> u64 {
    ...
    pub fn from_encoded_stream_with_hint<R: 'static + Read>(
            ...
            && decode_args.frame_rate_hz != original_frame_rate_hz
        {
            ...
            buffer = match resample(
                ...
                Err(err) => {
                    return match err {
                        babycat_lib_resample::errors::Error::WrongInputFrameRate(..) => Err(
                            Error::WrongFrameRate(original_frame_rate_hz, final_frame_rate_hz),
                        ),
                        babycat_lib_resample::errors::Error::WrongOutputFrameRate(..) => Err(
                            Error::WrongFrameRate(original_frame_rate_hz, final_frame_rate_hz),
                        ),
                        babycat_lib_resample::errors::Error::WrongNumChannels(..) => {
                            Err(Error::ResamplingError)
                        }
                        babycat_lib_resample::errors::Error::WrongFrameRate(..) => Err(
                            Error::WrongFrameRate(original_frame_rate_hz, final_frame_rate_hz),
                        ),
                        babycat_lib_resample::errors::Error::WrongFrameRateRatio(..) => Err(
                            Error::WrongFrameRateRatio(original_frame_rate_hz, final_frame_rate_hz),
                        ),
                        babycat_lib_resample::errors::Error::UnknownError(..) => {
                            Err(Error::ResamplingError)
                        }
                        babycat_lib_resample::errors::Error::FeatureNotCompiled => {
                            Err(Error::FeatureNotCompiled("resample"))
                        }
                    };
                }
            }
...
impl Waveform for FloatWaveform {
    fn frame_rate_hz(&self) -> u32 {
        self.frame_rate_hz
    }
